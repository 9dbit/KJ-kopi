    ...
    <main>
      <header className="nav shell">
        ...
        <nav>
          ...
          <a className="navCta" href="#products">Keranjang · {cartCount}</a>
        </nav>
      </header>
      ...
      <section id="products" className="products section"><div className="shell"><div className="productHead"><div><p className="eyebrow">SHOP KANG JAGO</p><h2>Pilih jagoanmu.</h2></div><span>WooCommerce-ready catalog</span></div><div className="productGrid">{products.map((p,i)=><article className="product" key={p.id || p.name}><button className="productVisual" type="button" onClick={() => setSelected(p)}>{p.image ? <img src={p.image} alt={p.name}/> : <div className={'miniPack p'+(i%3)}><small>KOPI</small><b>KANG<br/>JAGO</b><i>GINSENG</i></div>}</button><h3>{p.name}</h3><p>{p.note} • {p.weight}</p><div><strong>{p.price}</strong><button type="button" onClick={() => addToCart(p)} aria-label={`Tambah ${p.name}`}>+</button></div></article>)}</div></div></section>
      ...
      {selected && <div className="modalBackdrop" onClick={() => setSelected(null)}><section className="productModal" onClick={(e)=>e.stopPropagation()}><button className="close" onClick={()=>setSelected(null)}>×</button><p className="eyebrow">KOPI KANG JAGO</p><h2>{selected.name}</h2><p>{selected.note}</p><div className="modalFacts"><span>{selected.weight}</span><strong>{selected.price}</strong></div><button className="primary modalCta" onClick={()=>addToCart(selected)}>Tambah ke Keranjang</button></section></div>}
